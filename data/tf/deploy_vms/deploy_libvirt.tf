...
variable "vm_deploy_name" {}

variable "vm_deploy_type" {
  type = "map"
}

variable "vm_deploy_ip_address" {}

variable "vm_deploy_mac_address" {}
...
# deploy VMs
data "template_file" "vm_deploy_dhcp_network" {
  ...
  vars {
    ...
    ip_address   = "${var.vm_deploy_ip_address}"
    mac_address  = "${var.vm_deploy_mac_address}"
  }
}

output "network1-output1" {
  value = ["${data.template_file.vm_deploy_dhcp_network.*.rendered}"]
}

resource "null_resource" "run_vm_deploy_dhcp_network" {
  provisioner "local-exec" {
    command = "${data.template_file.vm_deploy_dhcp_network.rendered}"
  }
}
...
resource "libvirt_volume" "vm_deploy_root_volume" {
  name           = "${var.vm_deploy_name}_vol.img"
  pool           = "${var.vm_vol_pool_name}"
...
data "template_file" "vm_resize_deploy_root_volume" {
  ...
  vars {
    ...
    volume_name = "${var.vm_deploy_name}_vol.img"
    volume_size = "${var.vm_deploy_type["root_disk_size"]}"
  }
  depends_on = ["libvirt_volume.vm_deploy_root_volume"]
}
...
resource "libvirt_cloudinit" "cloud_init_deploy" {
  name               = "${var.vm_deploy_name}_cloudinit.iso"
  pool               = "${var.vm_vol_pool_name}"
  local_hostname     = "${var.vm_deploy_name}"
...
resource "libvirt_domain" "deploy-vms" {
  name       = "${var.vm_deploy_name}"
  vcpu        = "${var.vm_deploy_type["cpu"]}"
  memory     = "${var.vm_deploy_type["memory"]}"
  ...
  network_interface {
    network_name   = "${var.network1["name"]}"
    hostname       = "${var.vm_deploy_name}"
    addresses      = ["${var.vm_deploy_ip_address}"]
    mac            = "${var.vm_deploy_mac_address}"
    wait_for_lease = 1
  }
  ...
  disk {
    volume_id = "${libvirt_volume.vm_deploy_root_volume.id}"
    pool      = "${var.vm_vol_pool_name}"
  }
  graphics {
    type        = "spice"
